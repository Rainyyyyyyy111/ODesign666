    ...
    def predict(self, data: Mapping[str, Mapping[str, Any]]) -> dict[str, torch.Tensor]:
        ...
        for struct_samples in inv_samples: 
            ...
            for smp in struct_samples:  
                ...
                atom_indices_full = smp["res_atom_indices"]   # residue -> atom idx list
                ...
                for k, seq_out in enumerate(pred_seqs):
                    ...
                    if len(tokens) == L_full:
                        patch_tokens = [tokens[i] for i in design_pos.tolist()]
                    elif len(tokens) == n_design:
                        patch_tokens = tokens
                    else:
                        raise ValueError(
                            f"Unexpected seq length from inference: token_len={len(tokens)}, "
                    ...
                    merged_variants[k]["per_chain"][ch] = {
                        "atom_indices": atom_indices_patch,
                        "new_seq": seq_patch,
                    }
